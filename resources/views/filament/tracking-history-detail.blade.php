...
<div class="space-y-4">
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-900 dark:text-gray-300">{{ $record->created_at->format('Y-m-d H:i:s') }}</p>
        </div>
        <div>
            ...
            <p class="mt-1 text-sm">
                @php
                    $eventTypes = [
                        ...
                        'document_approved' => ['label' => 'Document Approved', 'color' => 'green'],
                        'document_rejected' => ['label' => 'Document Rejected', 'color' => 'red'],
                        'document_revision_needed' => ['label' => 'Document Revision Needed', 'color' => 'yellow'],
                        'stage_transition' => ['label' => 'Stage Changed', 'color' => 'purple'],
                        'status_change' => ['label' => 'Status Changed', 'color' => 'gray'],
                    ];
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Stage</h3>
            <p class="mt-1 text-sm text-gray-900 dark:text-gray-300">{{ $record->stage->name ?? 'N/A' }}</p>
        </div>
        <div>
    ...
    <div>
        ...
        <p class="mt-1 text-sm text-gray-900 dark:text-gray-300">{{ $record->processor->fullname ?? 'System' }}</p>
    </div>

    @if($record->document_id && $document)
    <div class="border rounded-lg p-4 bg-gray-50 dark:bg-gray-800">
        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Related Document</h3>
    ...
    <div>
        <h3 class="text-sm font-medium text-gray-500 dark:text-gray-400">Comment</h3>
        <div class="mt-1 p-2 border rounded-md bg-white dark:bg-gray-800">
            <p class="text-sm text-gray-900 dark:text-gray-300 whitespace-pre-line">{{ $record->comment ?? 'No comment provided' }}</p>
        </div>
    </div>
</div>